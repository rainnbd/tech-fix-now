<script>
    ...
    const cargarServicios = async () => {
      try {
        ...
        const querySnapshot = await db.collection('servicios').orderBy('creadoEl', 'desc').get();
        servicios = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      } catch (error) {
  ...
  <main>
    {#if usuario}
      <div class="pagina">
        <div class="botones">
          <button class="btn-publicar" on:click={publicarServicio}>Publicar Servicio</button>
          <button class="btn-ver" on:click={verPublicaciones}>Ver Publicaciones</button>
                ...
                {#if servicio.usuarioPublicador && servicio.usuarioPublicador.nombre !== usuario.displayName}
                  <div class="servicio">
                    <div class="detalles-servicio">
                      ...
                      <p><strong>Ubicación:</strong> {servicio.departamento}, {servicio.ciudad}</p>
                    </div>
                    ...
                    <div class="usuario-publicador">
                      <img src={servicio.usuarioPublicador.foto} alt="Foto del usuario" />
                      <p>{servicio.usuarioPublicador.nombre} ({servicio.usuarioPublicador.tipoCuenta})</p>
                      <button on:click={() => goto(`/chat/${servicio.id}`)}>Enviar Mensaje</button>
                    </div>
                  </div>
                {/if}
              {/each}
  ...
  <style>
    ...
    .btn-publicar, .btn-ver, .btn-chats {
      padding: 1rem 2rem;
      border: none;
      border-radius: 5px;
      cursor: pointer;
    ...
    .btn-publicar:hover, .btn-ver:hover, .btn-chats:hover {
