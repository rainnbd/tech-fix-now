<script>
    ...
    const cargarMensajes = async () => {
      try {
        ...
        const querySnapshot = await db.collection('chats').doc(servicioId).collection('mensajes').orderBy('enviadoEl', 'asc').get();
  
        mensajes = querySnapshot.docs.map((doc) => doc.data());
        mensajeCargando = mensajes.length > 0 ? '' : 'No hay mensajes en este chat.';
      } catch (error) {
    ...
    const enviarMensaje = async () => {
      ...
      try {
        const db = firebase.firestore();
        await db.collection('chats').doc(servicioId).collection('mensajes').add({
          texto: nuevoMensaje,
          usuario: {
            id: usuario.uid,
    ...
    {#if usuario}
      <div class="chat">
        ...
        <div class="mensajes">
          {#if mensajes.length > 0}
            {#each mensajes as mensaje}
              <div class="mensaje {mensaje.usuario.id === usuario.uid ? 'propio' : 'ajeno'}">
                <img src={mensaje.usuario.foto} alt="Foto del usuario" />
                <div class="texto">
                  <p><strong>{mensaje.usuario.nombre}:</strong> {mensaje.texto}</p>
                </div>
              </div>
            {/each}
          {:else}
        ...
        <div class="enviar-mensaje">
          <input type="text" bind:value={nuevoMensaje} placeholder="Escribe tu mensaje..." />
