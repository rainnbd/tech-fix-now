<script>
    ...
    const cargarPublicaciones = async () => {
      try {
        ...
        const querySnapshot = await db.collection('servicios').where('usuarioPublicador.nombre', '==', usuario.displayName).get();
  
        publicaciones = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        mensajeCargando = publicaciones.length > 0 ? '' : 'Aún no hay publicaciones.';
      } catch (error) {
            ...
            {#each publicaciones as publicacion}
              <div class="publicacion">
                <div class="detalles-publicacion">
                  ...
                  <p><strong>Ubicación:</strong> {publicacion.departamento}, {publicacion.ciudad}</p>
                </div>
                ...
                <div class="acciones">
                  <button on:click={() => editarPublicacion(publicacion.id)}>Editar</button>
                  <button on:click={() => eliminarPublicacion(publicacion.id)}>Eliminar</button>
                </div>
              </div>
            {/each}
          {:else}
