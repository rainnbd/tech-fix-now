<script>
  ...
  import { signOut, updateProfile, onAuthStateChanged, deleteUser } from 'firebase/auth';
  ...
  if (typeof window !== 'undefined') {
    onAuthStateChanged(auth, (user) => {
      if (user) {
        usuario = user;
        const partesNombre = user.displayName ? user.displayName.split(' ') : ['Usuario', ''];
  ...
  <div class="perfil">
    ...
    <div class="foto-container">
      ...
      <div class="botones-foto">
        <button class="btn-cambiar" on:click={() => document.querySelector('#uploadInput').click()}>Cambiar Foto</button>
        <button class="btn-eliminar" on:click={eliminarFoto}>Eliminar Foto</button>
      </div>
      <input id="uploadInput" type="file" accept="image/*" on:change={cambiarFoto} style="display: none;" />
    </div>
    ...
    <div class="info-container">
      <div class="info-item">
        ...
        <span>{correo}</span>
        <div class={correoVerificado ? 'estado-verificado' : 'estado-no-verificado'}>
      ...
      {#if tipoCuenta === 'Usuario'}
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="text" bind:value={nombre} placeholder="Nombre" class="input-text" />
          {:else}
            <span>{nombre}</span>
          {/if}
        </div>
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="text" bind:value={apellido} placeholder="Apellido" class="input-text" />
          {:else}
            <span>{apellido}</span>
          {/if}
        </div>
        ...
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="text" bind:value={ciudadMunicipio} placeholder="Ciudad o Municipio" class="input-text" />
          {:else}
        ...
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="text" bind:value={celular} placeholder="Celular" pattern="[0-9]*" class="input-text" />
          {:else}
            <span>{celular}</span>
          {/if}
        </div>
        ...
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="date" bind:value={fechaNacimiento} class="input-text" />
          {:else}
      ...
      {:else if tipoCuenta === 'Empresa'}
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="text" bind:value={nombre} placeholder="Nombre de la Empresa" class="input-text" />
          {:else}
            <span>{nombre}</span>
          {/if}
        </div>
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="text" bind:value={direccion} placeholder="Dirección" class="input-text" />
          {:else}
            <span>{direccion}</span>
          {/if}
        </div>
        ...
        <div class="info-item">
          ...
          {#if editandoDatos}
            <input type="text" bind:value={numeroContacto} placeholder="Número de Contacto" pattern="[0-9]*" class="input-text" />
          {:else}
    ...
    {#if editandoDatos}
      <div class="botones-edicion">
        ...
        <button class="btn-cancelar" on:click={() => (editandoDatos = false)}>Cancelar</button>
      </div>
    {:else}
      <button class="btn-editar" on:click={() => (editandoDatos = true)}>Editar Perfil</button>
    {/if}

    <button class="btn-eliminar-cuenta" on:click={eliminarCuenta}>Eliminar Cuenta</button>
...
<style>
  ...
  .btn-cambiar, .btn-eliminar, .btn-guardar, .btn-cancelar, .btn-editar, .btn-eliminar-cuenta, .btn-cerrar {
  ...
  .btn-eliminar, .btn-eliminar-cuenta {
    background-color: #e53e3e;
  }
  .btn-cancelar {
    background-color: #ff5e57;
  }
  .btn-cambiar:hover, .btn-eliminar:hover, .btn-guardar:hover, .btn-cancelar:hover, .btn-editar:hover, .btn-eliminar-cuenta:hover, .btn-cerrar:hover {
    opacity: 0.9;
  }
  .info-container {
    margin-top: 2rem;
